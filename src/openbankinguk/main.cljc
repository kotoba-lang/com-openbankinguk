(ns openbankinguk.main
  "Kotodama WASM entrypoint for the Openbankinguk clean-room actor (L4) — Clojure port.
  ...
  facts (`openbankinguk.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "openbankinguk")
(def tier "L4")
...
(def entity-specs
  [{:entity "Account"   :plural "accounts"   :id-prefix "openbank_acc"
    :fields [:holderId :iban :currency :balance]     :required [:iban :currency]
    :coerce {:balance :float}             :refs {}}
   {:entity "Consent"   :plural "consents"   :id-prefix "openbank_con"
    :fields [:accountId :scope :status :expiresAt]  :required [:scope :status]
    :coerce {}                            :refs {:accountId "Account"}}
   {:entity "Transaction" :plural "transactions" :id-prefix "openbank_tra"
    :fields [:accountId :amount :currency :bookedAt] :required [:amount :currency]
    :coerce {:amount :float}              :refs {:accountId "Account"}}
   {:entity "PaymentInitiation" :plural "paymentinitiations" :id-prefix "openbank_pay"
    :fields [:debtorAccount :creditorAccount :amount :status] :required [:debtorAccount :creditorAccount]
    :coerce {:amount :float}              :refs {}}
   {:entity "Institution" :plural "institutions" :id-prefix "openbank_ins"
    :fields [:name :bic :country]         :required [:name :bic]
    :coerce {}                            :refs {}}
   {:entity "Filing"    :plural "filings"    :id-prefix "openbank_fil"
    :fields [:institutionId :regime :period :status] :required [:regime :period]
    :coerce {}                            :refs {:institutionId "Institution"}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"openbankinguk.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "openbankinguk-compat" :tier tier :entities entities} 200])
...
(defn start! [] :openbankinguk-compat/ready)
